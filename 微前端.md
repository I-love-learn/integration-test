...
上面是解决 select 等其他 poperjs 错位的 el-table 中 tooltip 错位需要加
...
[无界微前端（wujie）：element-ui 弹框内使用 select 组件，弹出框位置异常解决方案\_wujie 嵌入页面 下拉样式异常-CSDN 博客](https://blog.csdn.net/weixin_40968646/article/details/129306272)

将 el-poper 从 fixed 定位换乘 absolute
...
这段配置是用于设置 HTTP 响应头，主要用于跨域资源共享（CORS, Cross-Origin Resource Sharing）的场景。以下是每个字段的具体解释：
...
**Access-Control-Allow-Origin:"\*" 不能和**Access-Control-Allow-Credentials: true 一起用 因为 前者无法携带 cookie 跨域需要具体域名
...
#### wujie 使用子应用时 wangeditor 不兼容

尝试了很多办法感觉最靠谱的还是不用它哈哈

#### wujie 使用 aieditor 踩坑

子应用使用 aieditor，会出现样式丢失的情况 即便加了 plugin 加载样式也无效 解决办法是把那个样式在子应用里重写一次 就是 root 的那个样式

#### wujie 子应用的执行方式是放在同源 iframe 里执行的 因此子应用的 local storage 会存在父应用下
...
https://wujie-micro.github.io/doc/question/#_1%E3%80%81%E8%AF%B7%E6%B1%82%E8%B5%84%E6%BA%90%E6%8A%A5%E9%94%99

#### 无界预加载原理

https://blog.csdn.net/qq_63358859/article/details/134858152

iframe 的 window.parent 可以获得主页面的 window

微前端 url 不能用 localhost 否则别人局域网访问时也会请求他本地的 localhost

#### 子应用依赖中给 window 变量赋值 但使用时子应用 window 取不到属性

打印发现属性是加在父 window 上了

https://www.cnblogs.com/crispyChicken/p/17913252.html

可以插件写在父应用（实测没生效） 或者在子应用中劫持 window
 ...
 jsBeforeLoaders: [
      {
        callback(appWindow) {
          ...
          const propsNeedAdjust = 'pdfjsLib'

          Object.defineProperty(appWindow, propsNeedAdjust, {
            get: function () {
              if (appWindow.__POWERED_BY_WUJIE__) {
                return appWindow.parent[propsNeedAdjust]
              } else {
                // 返回原始值
                return appWindow[propsNeedAdjust]
              }
            }
          })
        }
...
#### 踩坑：微前端项目放到主应用相同服务器文件夹下 微前端 url 地址与后端项目路径

后端把主应用与子应用放到同一域名（ip 下），通过文件目录区分。

比如 ip+/www 访问子应用，此时子应用运行时会微前端会访问 ip+/www 页面但其实子应用路由中并无/www 这个子路由导致加载不出来。

解决办法是子应用路由守卫配置一下 /www 导航到 子应用的 index 路由去

history 的原理就是让后端 ngix 转发 请求 router 路由 会当做文件 get 去请求拿不到 404 获取 index.html 然后再拿 url 地址去匹配路由。
