...
上面是解决select等其他poperjs错位的  el-table中tooltip错位需要加
...
[无界微前端（wujie）：element-ui 弹框内使用select组件，弹出框位置异常解决方案_wujie 嵌入页面 下拉样式异常-CSDN博客](https://blog.csdn.net/weixin_40968646/article/details/129306272)

将el-poper从 fixed定位换乘absolute
...
#### wujie子应用的执行方式是放在同源iframe里执行的 因此子应用的local storage 会存在父应用下
...
iframe的window.parent可以获得主页面的window

微前端url不能用localhost 否则别人局域网访问时也会请求他本地的localhost

#### 子应用依赖中给window变量赋值 但使用时子应用window取不到属性

打印发现属性是加在父window上了

https://www.cnblogs.com/crispyChicken/p/17913252.html

可以插件写在父应用（实测没生效）  或者在子应用中劫持window
